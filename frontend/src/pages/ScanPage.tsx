    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {servers.map(s => {
              ...
              return (
                <div key={s.id} onClick={() => toggleServer(s.id)}
                  ...
                  style={{ gridTemplateColumns: "auto 1fr auto", background: isChecked ? "#f0f7ff" : undefined }}>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid #f1f5f9" }}>
          <h2 className="font-display font-semibold text-sm" style={{ color: "#0f172a" }}>진단 로그</h2>
          <button onClick={() => setLogs([])} className="text-xs" style={{ color: "#94a3b8" }}>지우기</button>
        </div>
        <div className="font-mono text-xs p-4 overflow-y-auto" style={{ height: 220, color: "#374151", background: "#f8fafc" }}>
          {logs.length === 0 ? (
