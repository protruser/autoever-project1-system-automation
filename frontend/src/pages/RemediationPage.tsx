...
export default function RemediationPage() {
  ...
  const isOffline = selectedServer?.status !== "online";
  const toggleAll = () => { if (!isOffline) setChecks(p => p.map(c => ({ ...c, selected: !allSelected }))); };
  const toggleCheck = (id: string) => { if (!isOffline) setChecks(p => p.map(c => c.id === id ? { ...c, selected: !c.selected } : c)); };
  ...
  const runRemediation = async (targets: (VulnCheck & { selected: boolean })[]) => {
    ...
    try {
      ...
      addNotification({
        type: failCount === 0 ? "remediation_ok" : "remediation_fail",
        title: failCount === 0 ? "조치 완료" : "조치 일부 실패",
        body: `${selectedServer.hostname} — 성공 ${okCount}건, 실패/수동확인 ${failCount}건`,
      });
    } catch (e) {
      ...
      addNotification({
        ...
        title: "조치 요청 실패",
        body: `${selectedServer.hostname} 조치 요청이 실패했습니다: ${e instanceof Error ? e.message : String(e)}`,
      });
    }
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      {/* Host selector */}
      <div className="px-6 pt-4 pb-3 flex items-center gap-3 shrink-0" style={{ borderBottom: "1px solid var(--border)", background: "var(--card)" }}>
      ...
      {isOffline && selectedServer && (
        <div className="px-6 py-2.5 text-xs shrink-0" style={{ background: "#fffbeb", color: "#b45309", borderBottom: "1px solid var(--border)" }}>
          이 서버는 현재 오프라인 상태입니다. 온라인 상태에서만 조치를 실행할 수 있습니다.
        </div>
      )}

      {/* Action bar */}
      <div className="px-6 py-4 flex items-center gap-3 shrink-0" style={{ borderBottom: "1px solid var(--border)", background: "var(--card)" }}>
        <div onClick={toggleAll} className="w-5 h-5 rounded border flex items-center justify-center"
          style={{ background: allSelected ? "#1d4ed8" : "var(--card)", borderColor: allSelected ? "#1d4ed8" : "var(--border)", cursor: isOffline ? "not-allowed" : "pointer", opacity: isOffline ? 0.5 : 1 }}>
        ...
        <div className="flex gap-2 ml-auto">
          <button onClick={() => selectedChecks.length > 0 && setShowConfirm(true)}
            ...
            style={selectedChecks.length === 0 || applyState === "running" || isOffline ? { opacity: 0.4, cursor: "not-allowed" } : { boxShadow: "0 4px 14px rgba(29,78,216,0.25)" }}
            disabled={selectedChecks.length === 0 || applyState === "running" || isOffline}>
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          ...
          {!checksLoading && !checksError && checks.length === 0 && (
            <div className="text-center py-16 text-sm" style={{ color: "var(--text-tertiary)" }}>이 서버에는 취약/주의 항목이 없습니다.</div>
          )}
          {!checksLoading && !checksError && checks.map(c => {
            ...
            return (
              <div key={c.id} className="rounded-lg px-4 py-3 flex items-center gap-3 transition-all"
                style={{
                  background: c.selected ? "rgba(29,78,216,0.14)" : "var(--card)",
                  border: c.selected ? "1px solid #bfdbfe" : "1px solid var(--border)",
                }}>
                <div onClick={() => toggleCheck(c.id)}
                  className="w-5 h-5 rounded border flex items-center justify-center shrink-0"
                  style={{ background: c.selected ? "#1d4ed8" : "var(--card)", borderColor: c.selected ? "#1d4ed8" : "var(--border)", cursor: isOffline ? "not-allowed" : "pointer", opacity: isOffline ? 0.5 : 1 }}>
                ...
                <button onClick={() => runRemediation([c])} className="btn-secondary text-xs shrink-0 ml-1" disabled={applyState === "running" || isOffline}
                  style={isOffline ? { opacity: 0.4, cursor: "not-allowed" } : undefined}>
                  개별 조치
                </button>
              </div>
            );
        ...
        <div className="w-72 shrink-0 flex flex-col" style={{ borderLeft: "1px solid var(--border)" }}>
          ...
          <div className="flex-1 overflow-y-auto p-3 font-mono text-xs space-y-0.5" style={{ background: "var(--muted)" }}>
            {logs.length === 0 ? (
              <div className="text-center mt-12 text-xs" style={{ color: "var(--text-tertiary)" }}>조치 실행 시<br />로그가 표시됩니다.</div>
            ) : logs.map((l, i) => (
              <div key={i} style={{ color: l.type === "success" ? "#15803d" : l.type === "error" ? "#b91c1c" : "var(--text-secondary)" }}>{l.msg}</div>
            ))}
          </div>
          {applyState === "done" && (
